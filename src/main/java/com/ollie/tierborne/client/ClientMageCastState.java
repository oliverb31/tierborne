...
public final class ClientMageCastState {
    private static final Map<Integer, Animation> ANIMATIONS = new HashMap<>();

    private ClientMageCastState() {
    }

    public static void start(int entityId, int durationTicks, int style) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level == null) return;
        ANIMATIONS.put(entityId, new Animation(minecraft.level.getGameTime(),
                Math.max(1, durationTicks), Math.max(0, Math.min(2, style))));
    }
    ...
    public static int style(int entityId) {
        Animation animation = ANIMATIONS.get(entityId);
        return animation == null ? 0 : animation.style;
    }
    ...
    private record Animation(long startedAt, float durationTicks, int style) {
    }
}
