...
public final class PlayerMenuScreen extends Screen {
    ...
    private void renderClassSubclassSlot(PoseStack poseStack,int left,int top,int right,int bottom) {
        ...
        Skill subclass=current instanceof SwordsmanPlayerClass swordsman?swordsman.selectedSubclass(ClientProgress.unlockedSkills()):current instanceof ArcherPlayerClass archer?archer.selectedSubclass(ClientProgress.unlockedSkills()):current instanceof FighterPlayerClass fighter?fighter.selectedSubclass(ClientProgress.unlockedSkills()):null;
        ...
        switch(subclass.id()){
            ...
            case FighterPlayerClass.MONK->{details.add("Martial Arts: Empty main hand");details.add("Pull Range: "+one(ClientProgress.hasSkill(FighterPlayerClass.EXTENDED_PULL)?RpgBalanceConfig.PULL_UPGRADED_RANGE.get():RpgBalanceConfig.PULL_RANGE.get()));details.add("Pull Cooldown: "+one(ClientProgress.hasSkill(FighterPlayerClass.PULL_RECOVERY)?RpgBalanceConfig.PULL_UPGRADED_COOLDOWN_SECONDS.get():RpgBalanceConfig.PULL_COOLDOWN_SECONDS.get())+"s");}
