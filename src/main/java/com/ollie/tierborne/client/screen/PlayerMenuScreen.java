...
public final class PlayerMenuScreen extends Screen {
    ...
    private void renderClassPanel(PoseStack poseStack, int left, int top, int right, int bottom) {
        ...
        else if(playerClass instanceof MagePlayerClass){double magic=playerClass.totalBonus(SkillBonusType.MAGIC_DAMAGE,ClientProgress.unlockedSkills());if(ClientProgress.hasSkill(MagePlayerClass.FIRE_MAGE))magic+=RpgBalanceConfig.FIRE_MAGE_MAGIC_DAMAGE.get();if(ClientProgress.hasSkill(MagePlayerClass.ICE_MAGE))magic+=RpgBalanceConfig.ICE_MAGE_MAGIC_DAMAGE.get();if(ClientProgress.hasSkill(MagePlayerClass.LIGHTNING_MAGE))magic+=RpgBalanceConfig.LIGHTNING_MAGE_MAGIC_DAMAGE.get();stats.add("Magic Damage: "+signed(magic)+"%");stats.add("Staff Damage: "+one(RpgBalanceConfig.MAGE_STAFF_DAMAGE.get()));stats.add("Staff Range: "+one(RpgBalanceConfig.MAGE_STAFF_RANGE.get()));stats.add("Staff Cooldown: "+one(ClientProgress.hasSkill(MagePlayerClass.QUICK_FOCUS)?RpgBalanceConfig.MAGE_STAFF_QUICK_COOLDOWN_SECONDS.get():RpgBalanceConfig.MAGE_STAFF_COOLDOWN_SECONDS.get())+"s");}
    ...
    private void renderClassSubclassSlot(PoseStack poseStack,int left,int top,int right,int bottom) {
        ...
        Skill subclass=current instanceof SwordsmanPlayerClass swordsman?swordsman.selectedSubclass(ClientProgress.unlockedSkills()):current instanceof ArcherPlayerClass archer?archer.selectedSubclass(ClientProgress.unlockedSkills()):current instanceof FighterPlayerClass fighter?fighter.selectedSubclass(ClientProgress.unlockedSkills()):current instanceof BarbarianPlayerClass barbarian?barbarian.selectedSubclass(ClientProgress.unlockedSkills()):current instanceof MagePlayerClass mage?mage.selectedSubclass(ClientProgress.unlockedSkills()):null;
        ...
        switch(subclass.id()){
            ...
            case MagePlayerClass.FIRE_MAGE->{details.add("Magic Damage: +"+one(RpgBalanceConfig.FIRE_MAGE_MAGIC_DAMAGE.get())+"%");details.add("Arcane Burn: "+one(RpgBalanceConfig.FIRE_MAGE_BURN_SECONDS.get())+"s");if(ClientProgress.hasSkill(MagePlayerClass.FIREBALL_VOLLEY))details.add("Fireball Volley: Unlocked");if(ClientProgress.hasSkill(MagePlayerClass.FLAME_RING))details.add("Flame Ring: Unlocked");}
            case MagePlayerClass.ICE_MAGE->{details.add("Magic Damage: +"+one(RpgBalanceConfig.ICE_MAGE_MAGIC_DAMAGE.get())+"%");details.add("Arcane Slow: "+one(RpgBalanceConfig.ICE_MAGE_SLOW_SECONDS.get())+"s");details.add("Freezing Resistance: Active");if(ClientProgress.hasSkill(MagePlayerClass.ICE_LANCE))details.add("Ice Lance: Unlocked");if(ClientProgress.hasSkill(MagePlayerClass.FROST_NOVA))details.add("Frost Nova: Unlocked");}
            case MagePlayerClass.POISON_MAGE->{details.add("Arcane Poison: "+one(RpgBalanceConfig.POISON_MAGE_POISON_SECONDS.get())+"s");if(ClientProgress.hasSkill(MagePlayerClass.VENOM_BOLT))details.add("Venom Bolt: Unlocked");if(ClientProgress.hasSkill(MagePlayerClass.TOXIC_CLOUD))details.add("Toxic Cloud: Unlocked");}
            case MagePlayerClass.LIGHTNING_MAGE->{details.add("Magic Damage: +"+one(RpgBalanceConfig.LIGHTNING_MAGE_MAGIC_DAMAGE.get())+"%");details.add("Arcane Bolt: Chains once");if(ClientProgress.hasSkill(MagePlayerClass.CHAIN_LIGHTNING))details.add("Chain Lightning: Unlocked");if(ClientProgress.hasSkill(MagePlayerClass.THUNDERSTEP))details.add("Thunderstep: Unlocked");}
            case MagePlayerClass.DOCTOR->{details.add("Healing: +"+one(RpgBalanceConfig.DOCTOR_HEALING_BONUS_PERCENT.get())+"%");details.add("Arcane Bolt: Heals");if(ClientProgress.hasSkill(MagePlayerClass.HEALING_PULSE))details.add("Healing Pulse: Unlocked");if(ClientProgress.hasSkill(MagePlayerClass.PURGE))details.add("Purge: Unlocked");if(ClientProgress.hasSkill(MagePlayerClass.ADRENALINE))details.add("Adrenaline: Unlocked");}
        }
