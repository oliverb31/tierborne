...
public record ChoosePlayerClassPacket(String playerClassId) {
 ...
 public static void handle(ChoosePlayerClassPacket p,Supplier<NetworkEvent.Context> supplier){NetworkEvent.Context c=supplier.get();c.enqueueWork(()->{ServerPlayer player=c.getSender();if(player==null)return;PlayerProgressSavedData d=PlayerProgressSavedData.get(player.getServer());if(d.get(player.getUUID()).choosePlayerClass(p.playerClassId)){d.changed();TierborneEvents.applySkillEffects(player,d.get(player.getUUID()));}ModNetwork.sync(player);});c.setPacketHandled(true);}
}
