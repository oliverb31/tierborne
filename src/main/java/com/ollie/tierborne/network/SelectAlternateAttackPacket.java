...
public record SelectAlternateAttackPacket(String attackId){
 ...
 public static void handle(SelectAlternateAttackPacket p,Supplier<NetworkEvent.Context> s){NetworkEvent.Context c=s.get();c.enqueueWork(()->{ServerPlayer player=c.getSender();if(player==null)return;PlayerProgressSavedData d=PlayerProgressSavedData.get(player.getServer());if(d.get(player.getUUID()).selectAlternateAttack(p.attackId)){com.ollie.tierborne.combat.AbilityRuntime.cancelFireball(player);d.changed();ModNetwork.sync(player);ModNetwork.syncAbilities(player);}});c.setPacketHandled(true);}
}
