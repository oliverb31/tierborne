...
public record SyncAbilityStatePacket(List<AbilityStatus> statuses,float mainCharge,float offhandCharge,boolean dualWielding,boolean multislashActive){
 public static void encode(SyncAbilityStatePacket p,FriendlyByteBuf b){b.writeCollection(p.statuses,(buf,s)->{buf.writeUtf(s.name(),64);buf.writeVarInt(s.remainingTicks());buf.writeVarInt(s.totalTicks());buf.writeBoolean(s.active());});b.writeFloat(p.mainCharge);b.writeFloat(p.offhandCharge);b.writeBoolean(p.dualWielding);b.writeBoolean(p.multislashActive);}
 public static SyncAbilityStatePacket decode(FriendlyByteBuf b){return new SyncAbilityStatePacket(b.readList(buf->new AbilityStatus(buf.readUtf(64),buf.readVarInt(),buf.readVarInt(),buf.readBoolean())),b.readFloat(),b.readFloat(),b.readBoolean(),b.readBoolean());}
