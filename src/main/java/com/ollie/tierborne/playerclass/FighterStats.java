...
public final class FighterStats {
    ...
    public static double generalDamage(PlayerProgress p){double value=FighterPlayerClass.ID.equals(p.playerClassId())?RpgBalanceConfig.FIGHTER_ROOT_GENERAL_DAMAGE.get():0;if(p.hasSkill(FighterPlayerClass.CHAMPION_DAMAGE))value+=RpgBalanceConfig.FIGHTER_CHAMPION_PATH_DAMAGE.get();if(p.hasSkill(FighterPlayerClass.DUELIST_DAMAGE))value+=RpgBalanceConfig.FIGHTER_DUELIST_PATH_DAMAGE.get();if(p.hasSkill(FighterPlayerClass.DUELIST))value+=RpgBalanceConfig.DUELIST_DAMAGE.get();if(p.hasSkill(FighterPlayerClass.DUELIST_DAMAGE_UPGRADE))value+=RpgBalanceConfig.DUELIST_DAMAGE_UPGRADE.get();return value;}
    public static double fistDamage(PlayerProgress p){double value=FighterPlayerClass.ID.equals(p.playerClassId())?RpgBalanceConfig.FIGHTER_ROOT_FIST_DAMAGE.get():0;if(p.hasSkill(FighterPlayerClass.MONK_FIST))value+=RpgBalanceConfig.FIGHTER_MONK_PATH_FIST.get();if(p.hasSkill(FighterPlayerClass.MONK))value+=RpgBalanceConfig.MONK_FIST.get();if(p.hasSkill(FighterPlayerClass.MONK_FIST_UPGRADE))value+=RpgBalanceConfig.MONK_FIST_UPGRADE.get();return value;}
